...
void save_mobile( FILE *fp, MOB_INDEX_DATA *pMobIndex )
{
    ...
    fprintf( fp, "%d ",	          pMobIndex->defroll ); //Grayson Defroll 8 July 2004
...
// ROM OLC 	&& ( !IS_SET( pExit->rs_flags, EX_BASHPROOF ) )
		    ...
		    //  && ( !IS_SET( pExit->rs_flags, EX_PASSPROOF ) )  )
			    locks = 2; This commented out in an attempt to fix EX_NOPASS,
			    uncomment and remove changes to fix*/
			int locks = 0;
			if (	IS_SET(pExit->rs_flags, EX_ISDOOR)
			&& (	!IS_SET(pExit->rs_flags, EX_PICKPROOF) )
			...
			&& (	IS_SET(pExit->rs_flags, EX_PICKPROOF) )
			&& (	IS_SET(pExit->rs_flags, EX_NOPASS) ) )
				locks = 4; // End of EX_NOPASS attempted fix

#if 0 /* Removed for ROM OLC */
			if ( IS_SET( pExit->rs_flags, EX_ISDOOR )
			&& ( !IS_SET( pExit->rs_flags, EX_PICKPROOF ) )
		        && ( IS_SET( pExit->rs_flags, EX_BASHPROOF ) )
		        && ( !IS_SET( pExit->rs_flags, EX_PASSPROOF ) ) )
...
/*    fprintf( fp, "Recall      %d\n",         pArea->recall );  ROM OLC */
	    ...
	    sprintf( log_buf, "OLC : Area File Saved : %s", pArea->name );
	    log_string( log_buf );
    }
...
void do_asave( CHAR_DATA *ch, char *argument )
{
    ...
    if( !IS_BUILDER(ch) )
    {
	send_to_char( "Only Builders may save areas.\n\r", ch );
	return;
    }
    ...
    if( betalock )
    {
	send_to_char( "Not while in BETA test mode!\n\r", ch );
	return;
    }
	...
	if( !IS_OLCADMIN(ch) )
	{
		send_to_char( "Only OLC Administrators may save the world!\n\r", ch);
		return;
	}

      sprintf( buf, "OLC : ASAVE WORLD : %s", ch->name );
	log_string( buf );
	...
	log_string( "OLC : ASAVE WORLD : Completed" );
	send_to_char( "\n\rYou saved the world.\n\r", ch );
	return;
    }
