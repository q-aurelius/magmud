...
int find_spellremort( CHAR_DATA *ch, const char *name )
{
    /* finds a spell the character can cast if possible */
    int sn, found = -1;
...
void say_spell( CHAR_DATA *ch, int sn )
{
	...
	{ "fly",		"volatus"		},
	{ "mage",		"magi"		},
	{ "sanctuary",	"fiat asylum"	},
	{ "armor",		"armatus"		},
	{ "haste",		"acceleret"		},
	{ "continual light",		"fiat lux"		},
	{ "fresh",		"ima"		},
	{ "re",		"candus"	},
	{ "blast",		"flabra"		},
	{ "create",		"fiat"		},
	{ "critical",	"critica vulnera"		},
	{ "giant strength",		"robur gigantis"},
	{ "magic",		"magica"	},
	{ "protection good",	"praesidium a bonitata"	},
	{ "protection evil",	"praesidium a malo"	},
	{ "detect good",		"scientia bonitatis"	},
	{ "detect evil",		"scientia mali"			},
	{ "stone skin", 		"cutus saxo"			},
	{ "a", "a" }, { "b", "b" }, { "c", "q" }, { "d", "e" },
	{ "e", "z" }, { "f", "y" }, { "g", "o" }, { "h", "p" },
	{ "i", "u" }, { "j", "y" }, { "k", "t" }, { "l", "r" },
	{ "m", "w" }, { "n", "i" }, { "o", "a" }, { "p", "s" },
...
{
    int save;
    save = 10 + ( victim->level - level) - victim->saving_throw;
...
void do_activate( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' )
    {
	send_to_char( "{g<CCP>:SYNTAX ERROR.{w\n\r", ch );
	return;
    }
    ...
    || (!IS_NPC(ch) && !can_use_skill(ch,sn)))
    {
	send_to_char( "{g<CCP>:PROGRAM NOT FOUND.{w\n\r", ch );
	return;
    }

    if ( ch->position < skill_table[sn].minimum_position )
    {
	send_to_char( "{g<CCP>:PROCESSOR BUSY.{w\n\r", ch );
	return;
    }
    ...
    if (( class_lookup("cyborg") == ch->class ) || ( class_lookup("warrior") == ch->class ))
    {
	mana += mana/2;
    }
    else if (( class_lookup("mage") == ch->class ) || ( class_lookup("cleric") == ch->class ))
    {
	if ( race_lookup("dragon") == ch->race )
	   mana -= mana/4;

	if  ((race_lookup("drow") == ch->race && align < -300)
		|| (race_lookup("high elf") == ch-> race && align > 300) )
	   mana = mana/2;
    }

	//If ch is a remort and has more than 600 hours, 10% of hours are subtracted from mana costs
    if ( IS_REMORT(ch) && GET_HOURS(ch) >= 600 )

	mana -= GET_HOURS(ch)/10;


	//If ch has more than 500 hours mana costs are 90%
    if ( GET_HOURS(ch) >= 500 )
	mana -= mana/10;
	...
	if ( arg2[0] == '\0' )
	{
	    ...
	    {
		send_to_char( "{g<CCP>:PROGRAM TARGET UNSPECIFIED.{w\n\r", ch );
		return;
	    }
	}
	else
	{
	    ...
	    {
		send_to_char( "{g<CCP>:PROGRAM TARGET NOT FOUND.{w\n\r", ch );
		return;
	    }
	}
      ...
      {
          send_to_char( "{g<CCP>:PROGRAM MUST HAVE EXTERNAL TARGET.{w\n\r", ch );
          return;
      }

      if ( IS_AFFECTED(ch, AFF_CHARM) && ch->master == victim )
	{
	    send_to_char( "{g<CCP>:PROGRAM CANNOT BE INITIALIZED ON SLAVED DEVICES.{w\n\r",
		ch );
	    return;
	}
	...
	if ( !IS_NPC(ch) )
	{
	    ...
	    {
		send_to_char("{g<CCP>:NOT ON THAT TARGET.{w\n\r",ch);
		return;
	    }
	    check_killer(ch,victim);

	    if ((ch->level >= LEVEL_HERO) && (victim->level <= MAX_MORT_LEVEL) && !IS_NPC(victim))
	    {
		sprintf( log_buf, "%s has activated %s on a mortal, %s.", ch->name, skill_table[sn].name, victim->name );
	...
	else
	{
	    if ( ( victim = get_char_room( ch, target_name ) ) == NULL )
	    {
		send_to_char( "{g<CCP>:TARGET NOT FOUND.{w\n\r", ch );
		return;
	    }
	...
	{
	    send_to_char( "{g<CCP>:PROGRAM FOR SYSTEM USE ONLY.{w\n\r", ch );
	    return;
	}
	...
	if ( arg2[0] == '\0' )
	{
	    send_to_char( "{g<CCP>:PROGRAM TARGET OBJECT UNSPECIFIED.{w\n\r", ch );
	    return;
	}

	if ( ( obj = get_obj_carry( ch, target_name, ch ) ) == NULL )
	{
	    send_to_char( "{g<CCP>:PROGRAM TARGET OBJECT NOT FOUND.{w\n\r", ch );
	    return;
	}
	...
	if (arg2[0] == '\0')
	{
	    if ((victim = ch->fighting) == NULL)
	    {
		send_to_char("{g<CCP>:PROGRAM TARGET NOT SPECIFIED.{w\n\r",ch);
		return;
	    }
	...
	if (target == TARGET_CHAR) /* check the sanity of the attack */
	{
	    ...
	    {
		send_to_char("{g<CCP>:PROGRAM TARGET NOT WITHIN PARAMETERS.{w\n\r",ch);
		return;
	    }
	...
	{
	    send_to_char("{g<CCP>:TARGET NOT FOUND.\n\r",ch);
	    return;
	}
	break;
    ...
    {
	send_to_char( "{g<CCP>:SYSTEM RESOURCES NOT AVAILABLE.{w\n\r", ch );
	return;
    }
    ...
    /*
	 Calculate the Wait State based on Level:
	 Combat spells default to a wait of 12, Dracs to 20. (Combat/Drac/Other)
	 First bonus is at 50.    (8/16/8) ( -4)
	 Second bonus is at 100.  (4/12/4) ( -8)
	 Third bonus is at 150.   (2/8/2)  (-12)
	 Fourth Bonus is at 200.  (2/4/2)  (-16)
	 Fifth Bonus is at 250.   (2/2/2)  (-18)
	 Final Bonus is at 300, wait = 1.
    */
    if (IS_HERO(ch))
	WAIT_STATE( ch, 0 );
    else if (ch->level>=50 && ch->level<100)
    {
	tmp_ws = skill_table[sn].beats - 4;
	if (tmp_ws < 2)
	   tmp_ws = 2;
	WAIT_STATE( ch, tmp_ws );
    }
    else if (ch->level>=100 && ch->level<150)
    {
	tmp_ws = skill_table[sn].beats - 8;
	if (tmp_ws < 2)
	   tmp_ws = 2;
	WAIT_STATE( ch, tmp_ws );
    }
    ...
    {
	tmp_ws = skill_table[sn].beats - 12;
	if (tmp_ws < 2)
	   tmp_ws = 2;
	WAIT_STATE( ch, tmp_ws );
    }
    else if (ch->level>=200 && ch->level<250)
    {
	tmp_ws = skill_table[sn].beats - 16;
	if (tmp_ws < 2)
	   tmp_ws = 2;
	WAIT_STATE( ch, tmp_ws );
    }
    else if (ch->level>=250 && ch->level<300)
    {
	tmp_ws = skill_table[sn].beats - 18;
	if (tmp_ws < 2)
	   tmp_ws = 2;
	WAIT_STATE( ch, tmp_ws );
    }
    ...
    if ( number_percent( ) > get_skill(ch,sn) && !IS_HERO(ch) )
    {
	send_to_char( "{g<CCP>:SYSTEM INTERRUPTED.{w\n\r", ch );
...
void do_cast( CHAR_DATA *ch, char *argument )
{
	...
	if ( class_lookup("cyborg") == ch->class )
	{
		send_to_char( "Silly cyborg, you can't cast spells.\n\r", ch );
		send_to_char( "Syntax: Activate <program>\n\r", ch );
		return;
	}
	...
	if ( !IS_NPC(ch) )
	{
	    ...
	    if ((ch->level >= LEVEL_HERO) && (victim->level <= MAX_MORT_LEVEL) && !IS_NPC(victim))
	    {
		sprintf( log_buf, "%s has cast %s on a mortal, %s.", ch->name, skill_table[sn].name, victim->name );
		wiznet(log_buf,NULL,NULL,WIZ_SECURE,0,get_trust(ch));
		log_string( log_buf );
	    }
	}
    ...
    /*
	 Calculate the Wait State based on Level:
	 Combat spells default to a wait of 12, Dracs to 20. (Combat/Drac/Other)
	 First bonus is at 25.    (8/16/8) ( -4)
	 Second bonus is at 50.   (6/14/6) ( -6)
	 Fourth bonus is at 75.  (4/12/4) ( -8)
	 Fifth bonus is at 100.   (2/8/2)  (-12)
	 Sixth Bonus is at 125.  (2/4/2)  (-16)
	 Seventh Bonus is at 150.   (2/2/2)  (-18)
	 Final Bonus is at 200, wait = 1.
    */
    if (IS_HERO(ch))
	WAIT_STATE( ch, 0 );
    else if (ch->level>=25 && ch->level<50)
    {
	tmp_ws = skill_table[sn].beats - 4;
	if (tmp_ws < 2)
	   tmp_ws = 2;
    ...
    else if (ch->level>=50 && ch->level<75)
    {
	tmp_ws = skill_table[sn].beats - 6;
	if (tmp_ws < 2)
	   tmp_ws = 2;
	WAIT_STATE( ch, tmp_ws );
    }
    else if (ch->level>=75 && ch->level<100)
    {
	tmp_ws = skill_table[sn].beats - 8;
	if (tmp_ws < 2)
	   tmp_ws = 2;
    ...
    else if (ch->level>=100 && ch->level<120)
    {
	tmp_ws = skill_table[sn].beats - 12;
	if (tmp_ws < 2)
	   tmp_ws = 2;
    ...
    else if (ch->level>=125 && ch->level<150)
    {
	tmp_ws = skill_table[sn].beats - 16;
	if (tmp_ws < 2)
	   tmp_ws = 2;
...
void do_forcepower( CHAR_DATA *ch, char *argument )
{
    char arg1[MAX_INPUT_LENGTH];
    char arg2[MAX_INPUT_LENGTH];
    char log_buf[MAX_STRING_LENGTH];
	...
	if ( class_lookup("cyborg") == ch->class )
	{
		send_to_char( "Silly cyborg, you can't use The Force.\n\r", ch );
		send_to_char( "Syntax: Activate <program>\n\r", ch );
		return;
	}
	
	if ( class_lookup("jedi") != ch->class )
	{
		send_to_char( "You cannot use The Force.\n\r", ch );
		return;
	}
    ...
    || (!IS_NPC(ch) && !can_use_skill(ch,sn)))
    {
	send_to_char( "You don't know any Force powers by that name.\n\r", ch );
	return;
    }
    ...
    {
    default:
	bug( "Do_forcepower: bad target for sn %d.", sn );
	return;
	...
	if ( arg2[0] != '\0' && !is_name( target_name, ch->name ) )
	{
	    send_to_char( "You cannot use this Force power on another.\n\r", ch );
	    return;
	}
	...
	if ( arg2[0] == '\0' )
	{
	    send_to_char( "Upon what should the Force Power be used?\n\r", ch );
	    return;
	}
	...
	if (arg2[0] == '\0')
	{
	    if ((victim = ch->fighting) == NULL)
	    {
		send_to_char("Use the Force power on whom or what?\n\r",ch);
		return;
	    }
...
void do_assemble( CHAR_DATA *ch, char *argument )
{
    ...
    || (!IS_NPC(ch) && !can_use_skill(ch,sn)))
    {
	send_to_char( "You don't know how to assemble that.\n\r", ch );
	return;
    }
	...
	*/
    ...
    {
    default:
	bug( "Do_assemble: bad target for sn %d.", sn );
	return;
...
void spell_acid_blast( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
	...
	dam += level;
  	...
  	new_ac = GET_AC(victim,AC_EXOTIC);  //Grayson Spell AC Feb 2004
...
void spell_frost_blast( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
...
void spell_fire_blast( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
...
void spell_lightning_blast( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;
	int align, new_ac;
...
void spell_mage_blast( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;
	int align;
...
void spell_magic_bash( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;
	int align;
    ...
    {
	damage_old(ch,victim,dam/2,sn,DAM_BASH,TRUE);
    }
    else
    {
	if (IS_AFFECTED(victim,AFF_FLYING))
	{
	    	act("{y$n{y knocks you out of the sky with a powerful bolt of magic!{w",
			ch,NULL,victim,TO_VICT);
		act("{yYou blast $N{y, with a powerful bolt of magic and knock $M{y out of the sky!{w",ch,NULL,victim,TO_CHAR);
		act("{y$n{y knocks $N{y out of the sky with a powerful bolt of magic.{w",
			ch,NULL,victim,TO_NOTVICT);
		affect_strip(victim,skill_lookup("fly"));
		/*WAIT_STATE(victim, 3 * PULSE_VIOLENCE);*/  /* No magic bash lag when flying - Grayson 27 April 2003 */
		damage_old(ch,victim,dam*2,sn,DAM_BASH,TRUE);
	...
	{
	    	act("{y$n{y knocks you down with a powerful bolt of magic!{w",
			ch,NULL,victim,TO_VICT);
		act("{yYou blast $N{y, with a powerful bolt of magic and knock $M{y down!{w",ch,NULL,victim,TO_CHAR);
		act("{y$n{y knocks $N{y down with a powerful bolt of magic.{w",
			ch,NULL,victim,TO_NOTVICT);
		WAIT_STATE(victim, 2 * PULSE_VIOLENCE); /* Magic Bash Lag Drasticly Reduced - Grayson 27 April 2003 */
...
void spell_bless( int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
	...
	if (ch->class == class_lookup("demon hunter"))
	{
		if (victim != ch)
		{
			send_to_char("Your kind cannot cast this spell on others.\n\r",ch);
			return;	
		}	
	}	
...
void spell_call_lightning( int sn, int level,CHAR_DATA *ch,void *vo,int target)
{
    ...
    send_to_char( "The lightning of the Gods strikes your foes!\n\r", ch );
    act( "$n calls the lightning of the Gods to strike $s foes!",
	...
	if ( vch->in_room == ch->in_room )
	{
		...
		if (IS_AFFECTED(vch,AFF_FLYING))
		    dam += dam/2;
		new_ac = GET_AC(vch,AC_EXOTIC);  //Grayson Spell AC Feb 2004
		new_ac = new_ac * get_combat_value("m_ac");
		new_ac = new_ac / get_combat_value("d_ac");
		if (ch->level > 50 && vch->level > 50)
		{
		   	dam = dam + new_ac;	 // For above 50 chars, armor now absorbs damage
		}						// Grayson 21 Feb 2004   
...
void spell_calm( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
	...
	if (ch->class == class_lookup("jedi") && !IS_GOOD(ch))
	{
		send_to_char("Without a calm clear mind, how can you spread it to others?\n\r", ch);
		return;	
	}	
...
void spell_cancellation( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    if ( 	!IS_NPC(ch)
		&& !IS_NPC(victim)
		&& !( is_same_clan( ch, victim ) || is_same_group( ch, victim ) )
		&& is_safe_spell(ch,victim,FALSE) )
    {
	send_to_char( "Not on that target!\n\r", ch );
	return;
    ...
    if ( 	!( is_same_clan( ch, victim ) || is_same_group( ch, victim ) )
		&& !is_safe_spell(ch,victim,FALSE) )
	check_killer( ch, victim);
    ...
    if (check_dispel(level,victim,skill_lookup("improved invisibility")))
    {
        act("$n fades into existance.",victim,NULL,NULL,TO_ROOM);
        found = TRUE;
    }
    if (check_dispel(level,victim,skill_lookup("improved detection")))
        found = TRUE;    
	...
	if (check_dispel(level,victim,skill_lookup("rage")))
        found = TRUE;

	if (check_dispel(level,victim,skill_lookup("inflict pain")))
        found = TRUE;

	if (check_dispel(level,victim,skill_lookup("focus")))
        found = TRUE;

	if (check_dispel(level,victim,skill_lookup("absorb")))
        found = TRUE;
...
void spell_chain_lightning(int sn,int level,CHAR_DATA *ch, void *vo,int target)
{
	     ...
	     tmp_vict = next_vict)
	{
	  ...
	  if (!is_safe_spell(ch,tmp_vict,TRUE) && tmp_vict != last_vict)
	  {
	    ...
	    if (IS_AFFECTED(ch,AFF_FLYING))
		dam += dam/2;
	    if (IS_AFFECTED(tmp_vict,AFF_FLYING))
		dam += dam/2;
...
void spell_charm_person( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    {
	send_to_char( "They are to aggressive to be charmed!\n\r", ch );
	return;
    }
...
void gren_flash_grenade( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    OBJ_DATA *grenade;

    grenade = create_object( get_obj_index( OBJ_VNUM_FLASH_GRENADE ), 0 );
   	grenade->level = level;
    grenade->value[0] = level;

    obj_to_char(grenade,ch);
    act("$n has constructed $p.",ch,grenade,NULL,TO_ROOM);
    act( "You construct $p.", ch, grenade, NULL, TO_CHAR );
    return;
}

void gren_fire_bomb( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    OBJ_DATA *grenade;

    grenade = create_object( get_obj_index( OBJ_VNUM_FIRE_BOMB ), 0 );
   	grenade->level = level;
    grenade->value[0] = level*2/3;
...
void ls_lightsaber( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    OBJ_DATA *saber, *crystal;
	AFFECT_DATA *paf;
	int numdice;
	...
	if ( (crystal == NULL || crystal->item_type != ITEM_SABER_CRYSTAL))
    {
		send_to_char("You lack the proper components for building a lightsaber.\n\r",ch);
		return;
    }
	
	switch ( crystal->pIndexData->vnum )
    {
    	case OBJ_VNUM_RED_CRYSTAL:
    	saber = create_object( get_obj_index( OBJ_VNUM_RED_SABER ), 0 );
    	break;
    	case OBJ_VNUM_BLUE_CRYSTAL:
    	saber = create_object( get_obj_index( OBJ_VNUM_BLUE_SABER ), 0 );
    	break;
    	case OBJ_VNUM_GREEN_CRYSTAL:
    	saber = create_object( get_obj_index( OBJ_VNUM_GREEN_SABER ), 0 );
    	break;
    	case OBJ_VNUM_YELLOW_CRYSTAL:
    	saber = create_object( get_obj_index( OBJ_VNUM_YELLOW_SABER ), 0 );
		break;
		case OBJ_VNUM_VIOLET_CRYSTAL:
    	saber = create_object( get_obj_index( OBJ_VNUM_VIOLET_SABER ), 0 );    
    }
   	extract_obj(crystal);
   	saber->level = level;
    saber->value[1] = numdice;
    saber->value[2] = 2;
        
    obj_to_char(saber,ch);
    act("$n has constructed $p.",ch,saber,NULL,TO_ROOM);
    act( "You construct $p.", ch, saber, NULL, TO_CHAR );
    
    
    
    ch->move = 0;
    ch->mana = 0;
    act( "You are mentally and physically exhausted.", ch, NULL, NULL, TO_CHAR );
    return;
}
...
void spell_demonfire(int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    {
        victim = ch;
	send_to_char("The {Ddemons{w turn upon you!\n\r",ch);
    }
    ...
    if (victim != ch)
    {
	act("$n calls forth the demons of {rHell{w upon $N!",
	    ch,NULL,victim,TO_ROOM);
        act("$n has assailed you with the demons of {rHell{w!",
	    ch,NULL,victim,TO_VICT);
	send_to_char("You conjure forth the demons of {rHell{w!\n\r",ch);
    }
    dam = dice( level, 15 );
    dam += (ch->level/10);
    if ( victim->level > (level + 20 ))
...
void spell_dispel_evil( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    if ( IS_GOOD(victim) )
    {
	act( "$N is protected by $S holiness.", ch, NULL, victim, TO_ROOM );
	return;
    }
...
void spell_fireball( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
	...
	dam 	= dam_each[level] / 2;
    if (level>50)
    	dam = dice(level, 6); //better scaling fireball - Gray 21 Feb 2004
    if  ((race_lookup("drow") == ch->race && align < -300)
		|| (race_lookup("high elf") == ch-> race && align > 300) )
	dam += level;
	if ( saves_spell( level, victim, DAM_FIRE) )
...
void spell_faerie_fog( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
	...
	affect_strip ( ich, gsn_improv_invis			);
	affect_strip ( ich, gsn_mass_invis		);
	affect_strip ( ich, gsn_sneak			);
	REMOVE_BIT   ( ich->affected_by, AFF_HIDE	);
	REMOVE_BIT   ( ich->affected_by, AFF_INVISIBLE	);
	REMOVE_BIT   ( ich->affected2_by, AFF_IMPROV_INVIS	);
...
void spell_haste( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
	...
	if (ch->class == class_lookup("jedi") && victim != ch)
	{
		send_to_char("You cannot speed up others with The Force.",ch);
		return;	
	}
	...
	/*if (class_lookup("warrior") == victim->class
	  ||class_lookup("cyborg") == victim->class) af.duration = 2;*/
...

{
    ...
    heal = level + level/2; /* Grayson Heal Improvement 27 April 2003 */
...
void spell_renew( int sn, int level, CHAR_DATA *ch, void *vo,int target )
...

{
	...
	WAIT_STATE( victim, 24*PULSE_VIOLENCE );
    ...
    victim->hit += ( victim->max_hit - victim->hit )/2;

    victim->mana += ( victim->max_mana - victim->mana )/2;

    victim->move += ( victim->max_move - victim->move )/2;

    update_pos( victim );

    send_to_char( "You feel healthy and full of energy.\n\r", victim );
...
void spell_identify( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
	...
	switch (obj->value[0])
	{
    	...
    	case(WEAPON_PROJ)   : send_to_char("projectile.\n\r",ch);	break;
    	case(WEAPON_BTL)    : send_to_char("bat'telh.\n\r",ch);	break;
...
void spell_improv_invis( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    CHAR_DATA *victim;
    OBJ_DATA *obj;
    AFFECT_DATA af;
    ...
    if ( IS_AFFECTED(victim, AFF_INVISIBLE)
    	||IS_AFFECTED2(victim, AFF_IMPROV_INVIS) )
    {
    	act( "$n is already invisible.", victim, NULL, NULL, TO_CHAR );
    	return;
    }
    ...
    af.location  = APPLY_NONE;
    af.modifier  = 0;
    af.bitvector = AFF_IMPROV_INVIS;
    affect_to_char( victim, &af );
    send_to_char( "You fade out of existence.\n\r", victim );
    return;
}

void spell_improv_detect( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;

    if ( IS_AFFECTED2(victim, AFF_IMPROV_DETECT) )
    {
        if (victim == ch)
          send_to_char("You already have improved detection.\n\r",ch);
        else
          act("$N can already see invisible things.",ch,NULL,victim,TO_CHAR);
        return;
    }
    ...
    af.modifier  = 0;
    af.location  = APPLY_NONE;
    af.bitvector = AFF_IMPROV_DETECT;
    affect_to_char( victim, &af );
    send_to_char( "Your eyes {ytingle{w.\n\r", victim );
    if ( ch != victim )
		send_to_char( "Ok.\n\r", ch );
    return;
}
...
void spell_ray_of_truth (int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    ...
    if (victim != ch)
    {
        act("$n raises $s hand, and a blinding ray of {Ylight{w shoots forth!",
            ch,NULL,NULL,TO_ROOM);
        send_to_char(
	   "You raise your hand and a blinding ray of {Ylight{w shoots forth!\n\r",
	   ch);
    }
...
void spell_entangle( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
	...
	if (!IS_OUTSIDE(ch))
	{
		send_to_char("You cannot cast that spell indoors.\n\r", ch);
		return;	
	}
    ...
    if ( is_affected( victim, sn ) || IS_AFFECTED2(victim,AFF_ENTANGLE))
    {
          act("$N is already entangled.",ch,NULL,victim,TO_CHAR);
        return;
    }

	if ( class_lookup("druid") == victim->class )
	{
		chance = dice(1,10);
		if (chance >= 4)
		{
			if (victim != ch)
            	send_to_char("$n evades the creeping vines.\n\r",ch);
            send_to_char("You manage to evade the creeping vines.\n\r",victim);
            return;
        }		
	}
	...
	if (level<50) mod = -1;
		else if (level<100) mod = -2;
		else if (level<150) mod = -3;
		else if (level<200) mod = -4;
		else if (level<250) mod = -5;
		else if (level<300)	mod = -6;
		else if (level<350) mod = -7;
		else mod = -8;
    ...
    affect_to_char( victim, &af );
    send_to_char( "{gVines creep out of the ground and entagle your feet.{w\n\r", victim );
    act("{g$n's feet are entangled by the creeping vines.{w",victim,NULL,NULL,TO_ROOM);
    return;
}
...

{
	...
	send_to_char( "They are to aggressive to be summoned!\n\r", ch );
...
void spell_weaken( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
	...
	if (class_lookup("demon hunter") == ch->class)
	{
		if (victim->race != class_lookup("vampire") ||
			victim->race != class_lookup("wraith"))
			{
			send_to_char("Not on that target!\n\r", ch);
			return;
			}
...
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam,hp_dam,dice_dam,hpch, new_ac;
/*
    if (( race_lookup("red dragon") != ch->race ) && !IS_HERO(ch))
    {
	send_to_char( "Only Dragons may use Dragon magic!\n\r", ch);
	return;
    }
*/
    ...
    */
	...
	dam /= 2;
/*
    if ( class_lookup("warrior") == ch->class )
	dam /= 2;
*/
    if ( victim->level > level + 20 )
	dam /= 2;
...
void spell_fire_breath( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    int hpch, new_ac;
/*
    if (( race_lookup("dragon") != ch->race ) && !IS_HERO(ch))
    {
	send_to_char( "Only Dragons may use Dragon magic!\n\r", ch);
	return;
    }
*/
	...
	dam += (level/2 * ( get_combat_value("m_dmfb_dam") / get_combat_value("d_dmfb_dam") ) );
...
*/
    act("$n breathes out a {Wfreezing{w cone of {Wfrost{w!",ch,NULL,victim,TO_NOTVICT);
    act("$n breathes a {Wfreezing{w cone of {Wfrost{w over you!",
	ch,NULL,victim,TO_VICT);
...
*/
    act("$n breathes out a {gcloud {wof poisonous gas!",ch,NULL,NULL,TO_ROOM);
    act("You breath out a {gcloud{w of poisonous gas.",ch,NULL,NULL,TO_CHAR);
...
void prog_optical_blast( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;
	int new_ac;
...
void force_push( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;
	int align;
	...
	if (IS_AFFECTED(victim,AFF_FLYING))
	{
	    act("{c$n{c holds out a hand. As $e gestures, you fall to the ground!{w",
			ch,NULL,victim,TO_VICT);
		act("{cYou reach out with{W The Force{c and pull $N{c out of the sky!{w",ch,NULL,victim,TO_CHAR);
		act("{c$n{c holds out a hand. As $e{c gestures, $N{c falls from the sky.{w",
			ch,NULL,victim,TO_NOTVICT);
		affect_strip(victim,skill_lookup("fly"));
		/*WAIT_STATE(victim, 3 * PULSE_VIOLENCE);*/  /* No magic bash lag when flying - Grayson 27 April 2003 */
		//damage_old(ch,victim,dam*2,sn,DAM_BASH,TRUE);
	...
	{
	    act("{c$n{c holds out a hand. As $e{c gestures, you feel yourself pushed back onto the ground!{w",
			ch,NULL,victim,TO_VICT);
		act("{cYou reach out with{W The Force{c and push $N{c to the ground!{w",ch,NULL,victim,TO_CHAR);
		act("{c$n{c holds out a hand. As $e{c gestures, $N{c falls to the ground.{w.{w",
			ch,NULL,victim,TO_NOTVICT);
		WAIT_STATE(victim, 2 * PULSE_VIOLENCE); /* Magic Bash Lag Drasticly Reduced - Grayson 27 April 2003 */
		//damage_old(ch,victim,dam,sn,DAM_BASH,TRUE);
	      stop_fighting( victim, TRUE );
...
void force_pull_weapon(int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    OBJ_DATA *obj;
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    bool weap1, weap2;
    ...
    {
	act("{c$S{g weapon won't budge!{w",ch,NULL,victim,TO_CHAR);
	act("{cYou feel a pull at your weapon, but your it doesn't budge!{w",
	    ch,NULL,victim,TO_VICT);
	return;
    }

    act( "{c$n holds out a hand, and your weapon flies away from you and into $s grasp!{w",
	 ch, NULL, victim, TO_VICT    );
    act( "{cYou reach out with {WThe Force{c and pull $N's weapon away!{w",  ch, NULL, victim, TO_CHAR    );
    act( "{c$n reaches out and plucks $N's weapon from the air{c!{w",  ch, NULL, victim, TO_NOTVICT );
...
void force_protection( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
	int align;
    ...
    {
		if (victim == ch)
		  send_to_char("You are aleady protected by{W The Force{w.\n\r",ch);
		else
		  act("$N is aleady protected by{W The Force{w.",ch,NULL,victim,TO_CHAR);
		return;
    }
    ...
    af.duration  = level;
    af.location  = APPLY_AC;
    af.bitvector = AFF_FORCE_PROTECTION;
    af.modifier  = -80;
    ...
    send_to_char( "Your are protected by {WThe Force{w.\n\r", victim );
    return;
}

void force_absorb( int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
	int align;
    ...
    if ( IS_AFFECTED2(victim, AFF_FORCE_ABSORB) )
    {
	if (victim == ch)
	  send_to_char("You are already defended by {WThe Force{w.\n\r",ch);
    ...
    af.location  = APPLY_NONE;
    af.modifier  = 0;
    af.bitvector = AFF_FORCE_ABSORB;


    affect_to_char( victim, &af );
    //act( "$n is surrounded by a white aura.", victim, NULL, NULL, TO_ROOM );
    send_to_char( "You surround yourself with the power of{W The Force{w.\n\r", victim );
    return;
}

void force_lightning(int sn,int level,CHAR_DATA *ch,void *vo,int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;
	int align;
	...
	if (IS_GOOD(ch))
	{
		send_to_char( "You hold forth your hand and ... nothing happens.", victim );
		return;
	}
	...
	if (IS_AFFECTED2(victim, AFF_FORCE_ABSORB))
	{
		act("You hold out your hand. {BLightning{w flies from your fingertips toward $N{w.",ch,NULL,victim,TO_CHAR);
		act("$N is defended by{W The Force{w.",ch,NULL,victim,TO_CHAR);
		act("$n holds out $s hand. {BLightning{w arcs from $s fingertips, but dies into nothing as it approaches $N{w.",ch,NULL,victim,TO_NOTVICT);
		act("$n holds out %s hand. {BLightning{w arcs from $s fingertips, but it dies into nothing as it approaches you{w.",ch,NULL,victim,TO_VICT);
		act("You are defended by{W The Force{w.",ch,NULL,victim,TO_NOTVICT);
		return;
	}
	...
	ch->alignment = UMAX(-1000, ch->alignment - 100);
	
	
	act("You hold out your hand. {BLightning{w flies from your fingertips toward $N{w.",ch,NULL,victim,TO_CHAR);
	act("$n holds out $s hand. {BLightning{w arcs from $s fingertips, surrounding $N{w.",ch,NULL,victim,TO_NOTVICT);
	act("$n holds out %s hand. {BLightning{w arcs toward you from $s fingertips. You shake and spasm as the current cooks you from the inside{w.",ch,NULL,victim,TO_VICT);
    damage_old( ch, victim, dam, sn, DAM_LIGHTNING ,TRUE);
    return;
}

void force_drain_life( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    ...
    if (victim != ch)
        ch->alignment = UMAX(-1000, ch->alignment - 150);
    ...
    send_to_char("You feel your life being painfully ripped away!\n\r",victim);
    act("Reaching out with the{r power{w of {DThe Force{w you tear $N's life-force away{w.",ch,NULL,victim,TO_CHAR);
    damage_old( ch, victim, dam, sn, DAM_NEGATIVE ,TRUE);
...
void force_pain( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    CHAR_DATA *victim;
    OBJ_DATA *obj;
    AFFECT_DATA af;
    ...
    if (IS_AFFECTED(victim,AFF_FORCE_PAIN) || saves_spell(level,victim,DAM_NEGATIVE))
	return;
    af.where     = TO_AFFECTS2;
    af.type      = sn;
    af.level     = level;
    ...
    send_to_char( "Your body is racked with pain. You can hardly move.\n\r", victim );
    if ( ch != victim )
	act("$N spasms suddenly with uncontrollable pain.",ch,NULL,victim,TO_CHAR);
    return;
}

void force_rage(int sn, int level, CHAR_DATA *ch, void *vo,int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
	...
	if (IS_GOOD(ch))
	{
		send_to_char("You reflexively ease your rage.\n\r", ch);
		return;
	}
	
    if (IS_AFFECTED2(victim,AFF_FORCE_RAGE) || IS_AFFECTED(victim,AFF_BERSERK)
        )
    {
		if (victim == ch)
		  send_to_char("You are already in a frenzy.\n\r",ch);
    ...
    if (victim != ch)
    {
    	send_to_char("You cannot use this ability on others.\n\r", ch);
    	return;	
    }
    ...
    send_to_char("The{r power{w of{D The Force fuels your rage!\n\r",victim);
    act("$n's eyes {rburn{w with rage!",victim,NULL,NULL,TO_ROOM);
}

void force_choke( int sn, int level, CHAR_DATA *ch, void *vo, int target )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int dam;
	int align;
	...
	if (IS_AFFECTED2(victim, AFF_FORCE_ABSORB))
	{
		act("You reach out with{D The Force{w, but are unable to choke $N{w.",ch,NULL,victim,TO_CHAR);
		act("$N is defended by{W The Force{w.",ch,NULL,victim,TO_CHAR);
		act("$n dramatically holds out $s hand toward $N to no apparant result{w.",ch,NULL,victim,TO_NOTVICT);
		act("$n dramatically holds out $s hand, attempting to attack you through{W The Force{w.",ch,NULL,victim,TO_VICT);
		act("You are defended by{W The Force{w.",ch,NULL,victim,TO_NOTVICT);
		return;
	}
    ...
    {
		act("You reach out with{D The Force{w, and constrict $N's airways{w.",ch,NULL,victim,TO_CHAR);
		act("$n dramatically holds out $s hand, and $N begins to gasp, clutching at $S own throat{w.",ch,NULL,victim,TO_NOTVICT);
		act("$n dramatically holds out $s hand, and suddenly you are unable to breathe{w.",ch,NULL,victim,TO_VICT);
		damage_old(ch,victim,dam,sn,DAM_ENERGY,TRUE);
    }
    return;
}

void force_focus( int sn, int level, CHAR_DATA *ch, void *vo, int target)
{
    CHAR_DATA *victim;
    OBJ_DATA *obj;
    AFFECT_DATA af;
    ...
    send_to_char( "You feel{W The Force{w flowing through you.\n\r", victim );

    return;
}

void force_fear( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    CHAR_DATA *victim;
    OBJ_DATA *obj;
    AFFECT_DATA af;
    ...
    send_to_char( "You are filled with unnatural terror.\n\r", victim );
    if ( ch != victim )
	act("$N's face is suddenly filled with fear.",ch,NULL,victim,TO_CHAR);
    return;
}

void spell_holy_signet( int sn, int level, CHAR_DATA *ch, void *vo,int target )
{
    CHAR_DATA *vch;
    CHAR_DATA *vch_next;
    int dam, new_ac;

    send_to_char( "You conjure the defining symbol of your {Yfaith{w!\n\r", ch );
    act( "$n conjures the emblem of $s {Yfaith{w into the air.", ch, NULL, NULL, TO_ROOM );
	...
	{
	    if ( vch != ch && !is_safe_spell(ch,vch,TRUE) && 
	    	(vch->race == race_lookup("vampire") || vch->race == race_lookup("wraith"))
	    	 )
