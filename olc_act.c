...
AEDIT( aedit_vnum )
{
    ...
    if ( iupper  > MAX_VNUM )
    {
	send_to_char( "AEdit:  Upper vnum cannot be greater than ", ch );
	send_to_char( MAX_VNUM, ch );
	send_to_char( ".\n\r", ch );
    }
...
REDIT( redit_show )
{
    ...
    sprintf( buf, "Name:       {W[{w%s{W]{w\n\rArea:       {W[{r%5d{W]{w %s\n\r",
	    pRoom->name, pRoom->area->vnum, pRoom->area->name );
    strcat( buf1, buf );

    sprintf( buf, "{WVnum:       [{g%5d{W]{w\n\rSector:     [%s]\n\r",
...
bool change_exit( CHAR_DATA *ch, char *argument, int door )
{
	...
	if ( !can_redit( ch, get_room_index( value )->area ) )
	{
...
REDIT( redit_create )
{
    ...
    if ( value > MAX_VNUM )
    {
	send_to_char( "{wVnum must be less than ", ch );
	send_to_char( MAX_VNUM, ch );
	send_to_char( "{w\n\r", ch );
	return FALSE;
    }
	...
	case ITEM_GRENADE:
		sprintf( buf,
			...
			obj->value[1] != -1 ? skill_table[obj->value[1]].name
			                    : "none" );
		    send_to_char( buf, ch ); //Grayson Grenade - 12 June 2004
		break;
	case ITEM_PORTAL:
	    sprintf( buf,
	        "[v0] Charges:        [%d]\n\r"
...
bool set_obj_values( CHAR_DATA *ch, OBJ_INDEX_DATA *pObj, int value_num, char *argument)
{
    switch( pObj->item_type )
    {
	    ...
	    switch ( value_num )
	    {
            ...
            break;
		case ITEM_GRENADE: //Grayson Grenade 12 June 2004
		switch ( value_num )
	    {
	        default:
		    do_help( ch, "ITEM_STAFF_WAND" );
		    ...
		    break;//Grayson Grenade 12 June 2004
        case ITEM_SCROLL:
        case ITEM_POTION:
        case ITEM_PILL:
	    switch ( value_num )
...
MEDIT( medit_defroll )
{
    ...
    if ( argument[0] == '\0' || !is_number( argument ) )
    {
	send_to_char( "Syntax:  defroll [number]\n\r", ch );
	return FALSE;
    }

    pMob->defroll = atoi( argument );

    send_to_char( "Defroll set.\n\r", ch);
    return TRUE;
}
