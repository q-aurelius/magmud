...
void do_spells(CHAR_DATA *ch, char *argument)
{
    ...
    add_buf(buffer,"\n\r\n\rRacial Skills and Spells may not be included in this list\n\r");
...
void list_group_costs(CHAR_DATA *ch)
{
	...
	&&  group_table[gn].rating[ch->class] > 0
	&&  group_table[gn].rating[ch->class] < 99)
	{
...
int exp_per_level(CHAR_DATA *ch, int points)
{
    ...
    if (points < 40 && ch->version >= 9)
    {
  		tempval = pc_race_table[ch->race].class_mult[ch->class]*10;
  		return tempval;  	
    }
    else
    {
...
void check_improve( CHAR_DATA *ch, int sn, bool success, int multiplier )
{
	...
	if (number_percent() < chance)
	{
	    ...
	    send_to_char(buf,ch);
	    ch->pcdata->learned[sn]++;
	    xp_gain = 2 * skill_table[sn].rating[ch->class];
	    xp_gain += wis_app[get_curr_stat(ch,STAT_WIS)].xp_bonus;
	    if (GET_HOURS(ch) <= 1000)
	    {
			if (xp_gain > 10)
			{
			   xp_gain = wis_app[get_curr_stat(ch,STAT_WIS)].xp_bonus;
			   gain_exp(ch, 10 + xp_gain);
			}
			else
			{
			   //xp_gain += wis_app[get_curr_stat(ch,STAT_WIS)].xp_bonus;
	    ...
	    else if (GET_HOURS(ch)/100 > xp_gain)
			gain_exp(ch, xp_gain);
	    else
			gain_exp(ch, GET_HOURS(ch)/100);
	}
    }
