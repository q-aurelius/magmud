...
bool spec_cop( CHAR_DATA *ch )
{
    ...
    sprintf( buf, "%s is %s!  AFTER THEM!!! YER GONNA BURN FOR YOUR CRIMES!!",
	victim->name, crime );
    REMOVE_BIT(ch->comm,COMM_NOSHOUT);
    do_yell( ch, buf );
    ...
    act( "$n is shackled and taken away.", victim, NULL, NULL, TO_ROOM );
	char_from_room (victim);
	char_to_room (victim, location = get_room_index( ROOM_VNUM_JAIL ) );
    act( "$n arrives in chains.", victim, NULL, NULL, TO_ROOM );
    if ( ch != victim )
	act( "$n shackles you and hauls you off to jail.", ch, NULL, victim, TO_VICT );
    do_look( victim, "auto" );
    return TRUE;
}
