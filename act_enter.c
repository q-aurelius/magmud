...
{    
	...
	if (	!IS_HERO(ch) 
		&& (	ch->in_room->vnum == ROOM_VNUM_JAIL
			...
			|| ch->in_room->vnum == ROOM_VNUM_IMMINT) )
	{
		send_to_char("The Immortals prevent you from leaving...\n\r",ch);
		return;
	}
	...
	for ( d = descriptor_list; d; d = d->next )
	{
		...
		&&   can_see(d->character, ch) )
		{
			if (!IS_NPC(ch))
				...
				send_to_char( ch->short_descr, d->character );
			send_to_char( " steps into ", d->character );
			if ( can_see_obj( d->character, portal ) )
			   send_to_char( portal->short_descr, d->character );
			else
			   send_to_char( "something.", d->character );
			send_to_char( "\n\r", d->character );
		}
	}
	   ...
	   for ( d = descriptor_list; d; d = d->next )
	   {
		...
		&&   can_see(d->character, ch) )
		{
			if (!IS_NPC(ch))
				...
				send_to_char( ch->short_descr, d->character );
			send_to_char( " has arrived through ", d->character );
			if ( can_see_obj( d->character, portal ) )
			   send_to_char( portal->short_descr, d->character );
			else
			   send_to_char( "something.", d->character );
