...
void move_char( CHAR_DATA *ch, int door, bool follow )
{
	...
	if (IS_AFFECTED2(ch,AFF_ENTANGLE))
	{
		send_to_char( "The vines entangling your feet hold you in place.\n\r", ch );
		return;	
	}
	
    if (IS_SET(pexit->exit_info, EX_CLOSED)
    &&  (!IS_AFFECTED(ch, AFF_PASS_DOOR) || IS_SET(pexit->exit_info,EX_NOPASS))
    &&  !(IS_IMMORTAL(ch) || IS_SET(ch->info, INFO_GHOST)))
    {
    ...
    /* Leaving the room */
    location = ch->in_room;
    for ( looker = location->people; looker; looker = looker->next_in_room )
    {
	/* People we can see who are not sneaking (roomout) */
	    ...
	    && looker != ch )
	{
		...
		sprintf( buf, "%s leaves %s.\n\r", PERS(ch, looker), dir_name[door] );
		send_to_char( buf, looker );
	}
	/* People we can see who are sneaking and we have Holylight on */
	else if( can_see(looker, ch)
		   && IS_AFFECTED(ch, AFF_SNEAK)
		   && IS_SET(looker->act,PLR_HOLYLIGHT)
		   && IS_AWAKE(looker)
		   && looker != ch )
	{
		sprintf(buf, "{w[{WSNEAK{w] %s leaves %s.\n\r", PERS(ch, looker), dir_name[door]);
		send_to_char(buf, looker);
	}
    }
    ...
    for ( looker = location->people; looker; looker = looker->next_in_room )
    {
	/* People we can see who are not sneaking (roomin) */
	if( can_see(looker, ch) 
	    && !IS_AFFECTED(ch, AFF_SNEAK)
	    && IS_AWAKE(looker)
	    && looker != ch )
	...
	/* People we can see who are sneaking and we have Holylight on */
		   ...
		   && looker != ch )
	{
		sprintf( buf, "{w[{WSNEAK{w] %s has arrived.\n\r", PERS(ch, looker) );
		send_to_char( buf, looker );
	}
    }
	...
	&& weather_info.sunlight != SUN_DARK ) 
	{
		...
		damage(ch,ch,dam,TYPE_SUNLIGHT,DAM_SUNLIGHT,TRUE, FALSE); /* Grayson -- Parryable 4 May 2003 */	
	}
...
void do_clan_recall( CHAR_DATA *ch, char *argument )
{
	...
	for ( d = descriptor_list; d; d = d->next )
	{
		...
		{
			sprintf(buf, "%s{w fades from view to return to the tabernacle.\n\r", ch->name );
			send_to_char( buf, d->character );
		}
	}
	send_to_char( "You return to the tabernacle.\n\r", ch );
	char_from_room( ch );
	...
	for ( d = descriptor_list; d; d = d->next )
	{
		...
		&&   IS_AWAKE(d->character) )
		{
			sprintf(buf, "%s{w fades into view, drawn by the Tabernacle.\n\r", ch->name );
			send_to_char( buf, d->character );
		}
	}
	do_look( ch, "auto" );
	...
	{
	   sprintf(buf, "%s{w attempts to shift phase!\n\r", ch->name );
	   send_to_char( buf, d->character );
	}
    }
    ...
    if ( IS_HERO(ch) && (ch->clan == 0) )
    {
	if ( IS_IMMORTAL(ch) )
		location = get_room_index( ROOM_VNUM_CHAT );
	else
		location = get_room_index( ROOM_VNUM_HEROES );
    }
    else
    ...
    {
	send_to_char( "You attempt to shift phase but nothing happens.\n\r", ch );
	return;
    }
	...
	&&   can_see(d->character, ch) )
	{
	   sprintf(buf, "%s{w begins to blur away.\n\r", ch->name );
	   send_to_char( buf, d->character );
	}
    }
    char_from_room( ch );
	...
	&&   can_see(d->character, ch) )
	{
	   sprintf(buf, "%s{w shifts into the current phase.\n\r", ch->name );
	   send_to_char( buf, d->character );
	}
    }
    do_look( ch, "auto" );
...
void do_stand( CHAR_DATA *ch, char *argument )
{
	...
	if (obj == NULL)
	{
	    ...
	    affect_strip( ch, gsn_trance );
	    REMOVE_BIT( ch->affected2_by, AFF_FORCE_TRANCE );
	    ch->on = NULL;
	}
...
void do_recall( CHAR_DATA *ch, char *argument )
{
	...
	for ( d = descriptor_list; d; d = d->next )
	{
		...
		&&   IS_AWAKE(d->character) )
		{
			sprintf(buf, "%s{w fades from view to return to the place of their death.\n\r", ch->name );
			send_to_char( buf, d->character );
		}
	}
	send_to_char( "You return to the room of your death.\n\r", ch );
	char_from_room( ch );
	...
	for ( d = descriptor_list; d; d = d->next )
	{
		...
		&&   IS_AWAKE(d->character) )
		{
			sprintf(buf, "%s{w fades into view here at the place of their death.\n\r", ch->name );
			send_to_char( buf, d->character );
		}
	}
	do_look( ch, "auto" );
...
void do_train( CHAR_DATA *ch, char *argument )
{
	...
	else if ( !str_cmp( argument, "mch" ) )
    {
	if ( class_table[ch->class].attr_prime == STAT_MCH )
	    cost    = 1;
	stat	    = STAT_MCH;
	pOutput     = "mechanical aptitude";
    }

	else if ( !str_cmp( argument, "tch" ) )
    {
	if ( class_table[ch->class].attr_prime == STAT_TCH )
	    cost    = 1;
	stat	    = STAT_TCH;
	pOutput     = "technical aptitude";
    }
	...
	if ( ch->perm_stat[STAT_AGI] < get_max_train(ch,STAT_AGI))  
	    strcat( buf, " agi" );
	if ( ch->perm_stat[STAT_MCH] < get_max_train(ch,STAT_MCH))  
	    strcat( buf, " mch" );
	if ( ch->perm_stat[STAT_TCH] < get_max_train(ch,STAT_TCH))  
	    strcat( buf, " tch" );
	strcat( buf, " hp mana");
...
void do_trance( CHAR_DATA *ch, char *argument )
{
	...
	{
		send_to_char("What? You think you some kinda Jedi or sumpin?\n\r", ch);
		return;	
	}
	...
	if (argument[0] == '\0' && ch->on == NULL)
	{
	    send_to_char( "You go into a Jedi healing trance.\n\r", ch );
	    act( "$n goes into a Jedi healing trance.", ch, NULL, NULL, TO_ROOM );
	    ch->position = POS_SLEEPING;
	    //send_to_char("Print1\n\r", ch);
	    ch->position = POS_SLEEPING;
		//send_to_char("Print\n\r", ch);
		if (number_percent() < chance)
		{
			...
			af.type 	 = gsn_trance;
   		 	af.modifier  = 0;
   		 	af.duration  = -1;
   			af.bitvector = AFF_FORCE_TRANCE;
		
			affect_to_char( ch, &af );
			check_improve(ch,gsn_trance,TRUE,2);
			//send_to_char("Trance successful\n\r", ch);		
		}
		else
		{
			check_improve(ch,gsn_trance,FALSE,2);	
			//send_to_char("Trance unsuccessful\n\r", ch);
		}
	}
	else  /* find an object and sleep on it */
	{
	    ...
	    if (IS_SET(obj->value[2],SLEEP_AT))
	    {
		act("You go into a Jedi healing trance at $p.",ch,obj,NULL,TO_CHAR);
		act("$n goes into a Jedi healing trance at $p.",ch,obj,NULL,TO_ROOM);
	    }
	    else if (IS_SET(obj->value[2],SLEEP_ON))
	    {
	        act("You go into a Jedi healing trance on $p.",ch,obj,NULL,TO_CHAR);
	        act("$n goes into a Jedi healing trance on $p.",ch,obj,NULL,TO_ROOM);
	    }
	    else
	    {
		act("You go into a Jedi healing trance in $p.",ch,obj,NULL,TO_CHAR);
		act("$n goes into a Jedi healing trance in $p.",ch,obj,NULL,TO_ROOM);
	    }
	   // send_to_char("Print1\n\r", ch);
	    ch->position = POS_SLEEPING;
		//send_to_char("Print\n\r", ch);
    ...
    case POS_FIGHTING:
	send_to_char( "Are you nuts? You are FIGHTING!\n\r", ch );
	break;
    }
